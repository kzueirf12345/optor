...
bool optor::PiskaToolButton::OnMousePress  (const ::dr4::Event& event) {
    ...
    if (state_->hoveredWidget == this && event.mouseButton.button == pressButton_) {
        if (!isPressed_) {
            piskaState_->selectedTool = tool_;
        } else {
...
bool optor::PiskaToolButton::OnKeyboardPress(const dr4::Event& event) {
    ...
    if (event.key.sym == dr4::KeyCode::KEYCODE_ESCAPE && piskaState_->selectedTool == tool_ && tool_->IsCurrentlyDrawing()) {
        piskaState_->selectedTool->OnBreak();
        return true;
    }
...
void optor::PiskaToolButton::OnIdle() {
    if (piskaState_->selectedTool == tool_) {
        isPressed_ = true;
