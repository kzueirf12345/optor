...
optor::pp::Arrow::Arrow(dr4::Window* dr4Window, const ::pp::ControlsTheme& theme, ::pp::State* state) 
...
bool optor::pp::Arrow::OnMouseDown(const dr4::Event::MouseButton &evt) {
    if (evt.button == dr4::MouseButtonType::LEFT) {
        ...
        // Проверяем контрольные точки в первую очередь
...
bool optor::pp::Arrow::OnStartPoint(dr4::Vec2f relCoord) const {
    if (state_->selectedShape != this) return false;
    
    dr4::Vec2f center = startPoint_->GetPos() + dr4::Vec2f(startPoint_->GetRadius(), startPoint_->GetRadius());
    dr4::Vec2f dist = relCoord - center;
    float distance2 = dist.x * dist.x + dist.y * dist.y;
    float radius2 = startPoint_->GetRadius() * startPoint_->GetRadius();
...
bool optor::pp::Arrow::OnEndPoint(dr4::Vec2f relCoord) const {
    ...
    dr4::Vec2f center = endPoint_->GetPos() + dr4::Vec2f(endPoint_->GetRadius(), endPoint_->GetRadius());
    dr4::Vec2f dist = relCoord - center;
    float distance2 = dist.x * dist.x + dist.y * dist.y;
    float radius2 = endPoint_->GetRadius() * endPoint_->GetRadius();
...
void optor::pp::Arrow::UpdateControlPoints() {
    float radius = startPoint_->GetRadius();
    startPoint_->SetPos(GetStart() - dr4::Vec2f(radius, radius));
    endPoint_->SetPos(GetEnd() - dr4::Vec2f(radius, radius));
}
