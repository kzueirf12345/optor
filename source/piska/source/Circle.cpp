...
void optor::pp::Circle::SetRadius(::dr4::Vec2f radius) {
    circle_->SetRadius(radius);
    UpdateSelectRect();
}

bool optor::pp::Circle::OnMe(dr4::Vec2f relCoord) const {
    dr4::Vec2f distance = DistanceFromCenter2(relCoord);
    dr4::Vec2f rad = circle_->GetRadius();
    return distance.x / (rad.x * rad.x) + distance.y / (rad.y * rad.y) <= 1;
}
...
::dr4::Vec2f optor::pp::Circle::DistanceFromCenter2(dr4::Vec2f relCoord) const {
    dr4::Vec2f center = circle_->GetPos() + circle_->GetRadius();
    dr4::Vec2f dist = relCoord - center;
    return dr4::Vec2f(dist.x * dist.x, dist.y * dist.y);
}

void optor::pp::Circle::UpdateSelectRect() {
    selectRect_->SetPos(circle_->GetPos());

    selectRect_->SetSize(2 * circle_->GetRadius());
}
...
void optor::pp::Circle::ResizeBySide(dr4::Vec2f offset) {
    ...
    dr4::Vec2f radius = circle_->GetRadius();

    switch (activeSide_) {

        case Side::LEFT: {
            float newRadius = radius.x - offset.x / 2;
            if (newRadius > 0.0f) {
                radius.x = newRadius;
                pos.x += offset.x;
            }
        ...
        case Side::TOP: {
            float newRadius = radius.y - offset.y / 2;
            if (newRadius > 0.0f) {
                radius.y = newRadius;
                pos.y += offset.y;
            }
